...
pub fn test_audio_playback() -> Result<()> {
    ...
    let file = File::open(test_file).map_err(|e| anyhow::anyhow!("Failed to open file: {}", e))?;
...
mod tests {
    ...
    fn test_audio_playback() -> Result<()> {
        ...
        audio.register_track(
            "opening",
            AudioTrack::new("assets/sound/music/opening.mp3").with_volume(0.5),
        );
        audio.register_track(
            "combat",
            AudioTrack::new("assets/sound/music/combat.mp3").with_volume(0.6),
        );
