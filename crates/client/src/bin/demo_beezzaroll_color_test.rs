...
fn render_fg_named_colors(frame: &mut Frame, bg: Color, area: Rect) {
    ...
    let areas = Layout::vertical([Constraint::Length(1); 2]).split(inner);
    for area in areas.iter() {
        let cells = Layout::horizontal([Constraint::Ratio(1, 8); 8]).split(*area);
...
fn render_indexed_colors(frame: &mut Frame, area: Rect) {
    ...
    let mut index_layout = Vec::new();
    for &area in &[layout[2], layout[4]] {
        let columns = Layout::horizontal([Constraint::Length(27); 3]).split(area);
        for col in columns.iter() {
            let rows = Layout::vertical([Constraint::Length(1); 6]).split(*col);
            for row in rows.iter() {
                let cells = Layout::horizontal([Constraint::Min(4); 6]).split(*row);
                for cell in cells.iter() {
                    index_layout.push(*cell);
                }
            }
        }
    }
