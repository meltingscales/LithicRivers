...
class KeychordCapture:
        ...
        if existing_file.exists():
            try:
                with open(existing_file) as f:
                    ...
                    logger.info(
                        f"📂 Loaded {len(existing_data)} existing keychords from {existing_file}"
                    )
            except Exception as e:
                logger.warning(
                    f"⚠️ Could not load existing keychords from {existing_file}: {e}"
                )
        else:
    ...
    def start_capture(self, screen: Screen):
        ...
        for i, prompt in enumerate(prompts):
            ...
            if (
                prompt in self.captured_keychords
                and self.captured_keychords[prompt] is not None
            ):
                skipped_count += 1
                logger.info(
                    f"⏭️ Skipping '{prompt}' - already captured as {self.captured_keychords[prompt]}"
                )
                continue

            if not self._capture_single_keychord(
                screen, prompt, i + 1 - skipped_count, total_prompts - skipped_count
            ):
                break
    ...
    def _capture_single_keychord(
        self, screen: Screen, prompt: str, current: int, total: int
    ) -> bool:
        ...
        screen.print_at(
            "💡 If a key doesn't work, press ESC to skip, then edit the JSON manually",
            0,
            7,
        )
