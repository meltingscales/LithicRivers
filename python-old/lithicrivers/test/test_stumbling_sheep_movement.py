...
class TestStumblingSheepMovement(OptimizedTestCase):
    ...
    def setUp(self):
        ...
        SimpleRNG._test_mode = True  # Enable test mode for deterministic mocking

    def tearDown(self):
        """Clean up test fixtures."""
        from lithicrivers.game.rng import SimpleRNG

        if hasattr(SimpleRNG, "_test_mode"):
            delattr(SimpleRNG, "_test_mode")
    ...
    def test_stumbling_sheep_creation(self):
        ...
        self.assertEqual(sheep.position, VectorN.create(0, 0, 0))
    ...
    def test_stumbling_sheep_movement_with_world(self):
        ...
        entities_at_start = self.game.world.get_entities(VectorN.create(0, 0, 0))
        self.assertIn(sheep, entities_at_start)

        # Mock random to always return True (so sheep always moves)
        with patch("lithicrivers.game.rng.SimpleRNG.random", return_value=0.05):  # Less than 0.1
            with patch("lithicrivers.game.rng.SimpleRNG.choice", return_value=VectorN.create(1, 0, 0)):  # Move east
                # Process entity ticks through the game
                self.game.process_entity_ticks()

        # Verify sheep moved to new position
        entities_at_new = self.game.world.get_entities(VectorN.create(1, 0, 0))
        self.assertIn(sheep, entities_at_new)

        # Verify sheep is no longer at old position
        entities_at_old = self.game.world.get_entities(VectorN.create(0, 0, 0))
    ...
    def test_stumbling_sheep_movement_without_world(self):
        ...
        self.assertEqual(sheep.position, VectorN.create(1, 0, 0))
